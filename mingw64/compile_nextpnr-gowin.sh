...
pip install apycula
pip install apycula --upgrade

mkdir BUILDS
cd BUILDS

git clone https://github.com/YosysHQ/nextpnr.git nextpnr-gowin
cd nextpnr-gowin
rm -rf build
mkdir build
cd build
cmake .. -G "MinGW Makefiles" -DARCH="gowin" -DBUILD_PYTHON=OFF -DBUILD_GUI=OFF -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=$DIR -DUSE_IPO=OFF
